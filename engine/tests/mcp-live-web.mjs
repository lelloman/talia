...
import {spawn,execFileSync} from 'node:child_process';import {createInterface} from 'node:readline';import assert from 'node:assert/strict';
const [engine,port,token,denied,serial]=process.argv.slice(2);
if(serial&&!serial.startsWith('emulator-'))throw Error('emulator required');
const app='com.lelloman.talia.dashboard';const adb=(...args)=>execFileSync('adb',['-s',serial,...args],{encoding:'utf8',timeout:45000});let nativeTarget;
class MCP {
 constructor(file){this.seq=0;this.pending=new Map();this.p=spawn((process.env.TALIA_BIN_DIR||'/tmp/talia-p3-target/debug')+'/talia-mcp',['http://127.0.0.1:'+engine,file]);this.p.stderr.pipe(process.stderr);createInterface({input:this.p.stdout}).on('line',s=>{const r=JSON.parse(s);this.pending.get(r.id)?.(r);this.pending.delete(r.id);});}
...
try{
 ...
 if(serial){
  adb('install','-r','dashboard/android/app/build/outputs/apk/debug/app-debug.apk');adb('reverse','tcp:'+port,'tcp:'+port);adb('shell','am','start','-W','-n',app+'/.MainActivity','--ei','port',port,'--ez','durable','true','--es','dashboard','monitor');
  await eventually(async()=>{const c=(await m.tool('clients_list')).clients.find(c=>c.platform==='android');const slot=c?.slots.find(s=>s.connected&&s.report.lifecycle==='active');if(slot)nativeTarget={clientId:c.clientId,slotId:slot.slotId,liveInstanceId:slot.liveInstanceId};return !!nativeTarget;});
  assert.equal((await m.tool('live_inspect',{target:nativeTarget})).snapshot.dirty,false);
 }
 ...
 if(nativeTarget)assert.equal((await m.tool('live_inspect',{target:nativeTarget})).snapshot.dirty,false);
 ...
 if(nativeTarget){
  const edit=await m.tool('live_execute',{target:nativeTarget,expectedEditRevision:0,source:'const s=ctx.state();await ctx.commit(s,{...s.value,details:false});',requestId:'cross-native-edit'});assert.equal(edit.audit.status,'complete');
  assert.equal((await m.tool('live_inspect',{target:nativeTarget})).snapshot.dirty,true);
  assert.equal(await other.evaluate(()=>dashboardReport.dirty),false);assert.equal(await other.evaluate(()=>dashboardReport.state.details),true);
  console.log(JSON.stringify({passed:true,checks:['simultaneous two-tab and native Android isolation','MCP browser edit leaves Android clean','MCP Android state edit leaves other tab unchanged']}));
 }
...
}finally{await browser.close();m.close();limited.close();if(serial){for(const args of [['shell','am','force-stop',app],['uninstall',app],['reverse','--remove','tcp:'+port]])try{adb(...args)}catch{}}}
