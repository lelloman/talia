...
fn v4_upgrade_preserves_monitoring_and_variable_state() {
    ...
    s.conn.execute_batch("DROP TABLE authored_definitions; DROP TABLE dashboard_packages; DELETE FROM metadata WHERE key='catalog_revision'; PRAGMA user_version=4;").unwrap();
    drop(s);
    let mut s = Store::open(&path).unwrap();
    assert_eq!(s.instance("metric").unwrap(), original);
    assert_eq!(s.monitoring_config().unwrap(), c);
