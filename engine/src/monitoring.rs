...
pub(crate) mod tests {
    ...
    fn sqlite_v1_upgrade_preserves_existing_values() {
        ...
        s.conn
            .execute_batch(
                "DROP TABLE ai_account; DROP TABLE monitoring_config; DROP TABLE monitoring_state; DROP TABLE monitoring_runs; DROP TABLE monitoring_requests; DROP TABLE monitoring_events; DELETE FROM metadata WHERE key='run_sequence'; DROP TABLE alert_entities; DROP TABLE alert_audit; DROP TABLE alert_requests; PRAGMA user_version=1;",
            )
            .unwrap();
        drop(s);
        let mut s = Store::open(&path).unwrap();
