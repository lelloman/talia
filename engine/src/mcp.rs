...
pub const INSTRUCTIONS:&str="Talìa saved authoring v1. Read catalog sources and revision, edit source/configuration, validate an atomic changeSet, then save with a fresh requestId and expectedCatalogRevision. On conflict reread/reconcile; never blindly retry a changed bundle. No tools currently execute engine actions or edit live VMs. Source document fields: ui/vm/function {source:string,references?:[{kind,id}]}; dashboard {ui:string,view_model:string,references?:[{kind,id}],params?:object,grants?:{reads:[],writes:[],runs:[]}}. UI uses Dashboard/Surface/Screen with Row/Column/Scroll containers and Text/Button/etc. Views; shared UI uses <Use definition=\"id\" .../>. JS calls defineVM; shared VM source is a VM object expression; shared function source is a function expression. Config kinds use the canonical engine schemas documented in docs/authoring-storage.md and docs/monitoring.md. Documents never contain resolved secrets. Validate returns safe compiler messages; runtime correctness still requires qualification. Limits: 256 changes, 2 MiB request/read, 128 KiB source, 256 KiB compiled package, 64 reference depth, 2048 records/8 MiB catalog. Credentials and deployment grant ceiling are operator-owned.";
