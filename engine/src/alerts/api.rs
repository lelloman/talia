...
pub fn tools() -> Vec<Value> {
 ...
 ("destination_save","Save a named email/telegram/push/web_push destination referencing an operator-owned provider. Push and web_push target device:ID, user:OWNER or group:NAME.",json!({"destination":{"type":"object","description":"{id,version,channel,provider,target,enabled?}"},"expected":number}),vec!["destination","expected"],false),
 ...
 ("device_register","Register/rotate this principal's installation push address. Owner comes from authentication; groups cannot be self-assigned.",json!({"device":{"type":"object","description":"{id,version,owner,token,channel? (default push),enabled?}"},"expected":number}),vec!["device","expected"],false),
