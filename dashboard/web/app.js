...
async function refresh(){
 ...
 try{const snapshot=TaliaValue.parse(await guest.eval('TaliaValue.stringify(TaliaVM.snapshot())'));if(stamp!==generation||revision!==uiRevision)return;if(snapshot.failure){fail(snapshot.failure);return;}state=snapshot.state;
  ...
  dirty.hidden=!snapshot.dirty;window.dashboardReport={...snapshot,stateWire:TaliaValue.encode(snapshot.state),connection:durable?engineClient.state:null,revision:loaded.revision,client:structuredClone(config),width:root.clientWidth-24,subscriptions:bridge.subscriptions.size,actions:[...bridge.actions.values()],actionIds:[...bridge.actions.keys()],nextActionId:bridge.nextActionId,externalError:bridge.externalError??null};
...
window.talia={renderer,refresh,command,checkUpdates,reload:start,async selectDashboard(id){if(!/^[A-Za-z][A-Za-z0-9_-]{0,63}$/.test(id))throw Error('invalid dashboard ID');config.dashboardId=id;localStorage.setItem('talia.client',JSON.stringify(config));try{await start();}catch(e){fail(e);throw e;}},async live(source){await command('TaliaVM.markDirty();\n'+source+"\n;'ok';");},get state(){return state;}};
