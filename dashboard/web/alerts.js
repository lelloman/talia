...
export async function alertRequest(op,args={}){
 ...
 const response=await fetch('/alerts',{method:'POST',headers:{'Content-Type':'application/json',...(!window.taliaOidc?{Authorization:'Bearer '+token}:{})},body:JSON.stringify({op,args,...(window.taliaOidc?{dashboard:window.taliaDashboard}: {})}),signal:AbortSignal.timeout(5000)});
 ...
 async function refresh(){if(!panel.open||panel.hidden||panel.closest('[data-shell-page]')?.hidden||document.hidden)return;
  try{const state=await alertRequest('snapshot');list.replaceChildren();status.textContent='';
   for(const a of state.alerts){const item=node('article','');item.append(node('h3',a.key),node('p',`${a.severity} · ${a.active?'Active':'Resolved'}${a.acknowledgement?' · Acknowledged by '+a.acknowledgement.actor:''}`),node('p',a.message));
    ...
    if(!window.taliaViewer)item.append(button('Silence for 1 hour',()=>alertRequest('silence_save',{expected:0,silence:{id:crypto.randomUUID(),version:1,key:a.key,until:state.now+3600000,reason:'Dashboard maintenance'}})));
   ...
   for(const s of state.silences.filter(s=>s.until>state.now)){const item=node('p',`Silenced: ${s.key||JSON.stringify(s.labels)} until ${new Date(s.until).toLocaleString()}`);if(!window.taliaViewer)item.append(button('End silence',()=>alertRequest('silence_save',{expected:s.version,silence:{...s,version:s.version+1,until:0}})));list.append(item);}
