...
export async function startTelegram(account){
 ...
 async function refresh(){
  ...
  $('telegram-investigations').checked=state.investigations;
  $('telegram-investigations').disabled=!state.investigationsAvailable&&!state.investigations;
  $('telegram-sources').value=state.sources.join(', ');
  $('telegram-ai-status').textContent=state.ai.configured?'simple-ai model: '+state.ai.model:state.ai.error;
  ...
  $('telegram-jobs').replaceChildren(...state.jobs.map(j=>{const p=document.createElement('p');p.textContent=`Investigation ${j.id}: ${j.status}${j.error?' · '+j.error:''}${j.run?' · Run '+j.run:''}`;return p;}));
 }
