...
import json,subprocess,sys,time,re,xml.etree.ElementTree as ET,tempfile,pathlib,shlex
...
def live(source):adb('shell','am','start','-n',package+'/.MainActivity','--es','live',shlex.quote(source))
try:
 ...
 tap('Reload dashboard');wait(lambda r:r.get('definitionRevision')=='revision-2' and r.get('dirty') is False and 25 in r.get('state',{}).get('history',[]))
 assert any(n.get('text')=='Updated shared notice' for n in hierarchy().iter('node'))
 live("throw Error('internal test failure')")
 wait(lambda r:bool(r.get('failure')),allow_failure=True)
 tap('Restart dashboard');r=wait(lambda r:not r.get('failure') and r.get('dirty') is False and 25 in r.get('state',{}).get('history',[]),allow_failure=True)
 print(json.dumps({'passed':True,'serial':serial,'checks':['native controls, no WebView','switch and condition','server write','accessible chart','navigation retains VM/subscription','per-client composition','update available without adoption','reload adopts reference revision and discards dirty VM','manual recovery preserves server effects'],'report':r}))
finally:
