...
root=pathlib.Path(tempfile.mkdtemp(prefix='talia-p1-avd-'));avd=root/'TaliaP1.avd';avd.mkdir()
...
p=None
try:
 p=subprocess.Popen([str(sdk/'emulator/emulator'),'-avd','TaliaP1','-port','5570','-no-window','-no-snapshot','-no-audio','-no-boot-anim','-gpu','swiftshader_indirect'],env={**os.environ,'ANDROID_AVD_HOME':str(root),'ANDROID_HOME':str(sdk)})
 result=p.wait()
