...
for(const p of hosts){
 ...
 const ctx={params:p,now:()=>1790183815800,source:async(_,q)=>{queries.push(q);return {result:q.query.startsWith('up')?[{samples:[[1,1]]}]:q.query.includes('filesystem')?[disk]:[{samples:[[1,20],[2,22]]}]};},publish:async(key,v)=>{assert.equal(key,'summary');published=v;}};
 ...
 const ui=TaliaUI.compile(doc.ui),definitions={'host-layout':TaliaUI.compileDefinition(source('host-layout')),'homelab-metric-card':TaliaUI.compileDefinition(readFileSync('dashboard/examples/homelab/metric-card.ui','utf8'))};
