...
assert.equal(changes.find(c=>c.key.kind==='monitor_definition'&&c.key.id==='host-collect').document.version,4);
...
for(const p of hosts){
 ...
 const ctx={params:p,now:()=>1790183815800,source:async(_,q)=>{queries.push(q);return {result:q.kind==='range'?[{samples:[[q.start+q.step,20],[q.end,22]]}]:q.query.startsWith('up')?[{samples:[[1,1]]}]:q.query.includes('filesystem')?[disk]:q.query.startsWith('node_memory_MemTotal_bytes')?[{samples:[[1,total]]}]:q.query.startsWith('node_memory_MemAvailable_bytes')?[{samples:[[1,available]]}]:[{samples:[[1,20],[2,22]]}]};},publish:async(key,v)=>{assert.equal(key,'summary');published=v;}};
 await collect.run(ctx);
 assert.equal(queries.length,8);for(const q of queries){assert.ok(q.query.includes('job='+JSON.stringify(p.job)));assert.ok(q.query.includes('instance='+JSON.stringify(p.instance)));}
