...
try{
 ...
 for(const [i,p] of hosts.entries()){
  ...
  for(const width of [390,1280]){
   ...
   assert.equal(await page.getByRole('radio',{name:'Show past 24 hours of CPU',checked:true}).count(),1);
   ...
   if(i===0&&width===390){await page.getByRole('radio',{name:'Show past hour of CPU',exact:true}).check();assert.equal(await page.evaluate(()=>window.lastAction),'cpuHour');const hourState=JSON.parse(JSON.stringify({ready:true,presentation:present(sample,p.host,'hour')}));await page.evaluate(({ui,definitions,state})=>renderer.render(TaliaUI.resolve(ui,state,{definitions,width:document.querySelector('#canvas').clientWidth})),{ui,definitions,state:hourState});await page.getByText('Past hour · 1-minute CPU averages',{exact:true}).waitFor();assert.equal(await page.getByRole('radio',{name:'Show past hour of CPU',checked:true}).count(),1);await page.screenshot({path:'.local/host-template/'+p.host+'-hour-390.png',fullPage:true});}
  }
 }
 ...
 // Each mounted Vue control must retain focus and have an independent radio name.
 await page.evaluate(()=>{window.controlTree={type:'Column',id:'controls',props:{},children:['first','second'].map(id=>({type:'SegmentedControl',id,props:{label:id},children:['day','hour'].map(value=>({type:'Button',id:id+'/'+value,props:{text:value,selected:value==='day',onClick:{action:value}},children:[]}))}))};renderer.render(window.controlTree);});
 await page.getByRole('radiogroup',{name:'second',exact:true}).getByRole('radio',{name:'hour',exact:true}).check();
 assert.equal(await page.getByRole('radiogroup',{name:'first',exact:true}).getByRole('radio',{name:'day',exact:true}).isChecked(),true);
 assert.equal(await page.evaluate(()=>window.lastAction),'hour');
 assert.equal(await page.evaluate(()=>{const focused=document.activeElement;controlTree.children[1].children.forEach(c=>c.props.selected=c.props.text==='hour');renderer.render(controlTree);return document.activeElement===focused&&focused.isConnected;}),true);
 await page.evaluate(()=>renderer.render({type:'Column',id:'empty',props:{},children:[]}));assert.equal(await page.locator('.lv-segmented').count(),0);
