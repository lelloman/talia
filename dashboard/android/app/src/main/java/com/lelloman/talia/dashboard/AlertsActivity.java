...
public final class AlertsActivity extends Activity {
 ...
 @Override public void onCreate(Bundle state){super.onCreate(state);port=getIntent().getIntExtra("port",18744);focusKey=getIntent().getStringExtra("alertKey");
  ...
  Button connect=new Button(this);connect.setText("Connect alerts");root.addView(connect);connect.setOnClickListener(v->{if(token.length()>0){getSharedPreferences("talia-alerts",0).edit().putString("credential",token.getText().toString()).apply();token.setText("");}AlertPush.schedule(this);refresh();});
  Button push=new Button(this);push.setText("Enable push notifications");root.addView(push);push.setOnClickListener(v->{if(android.os.Build.VERSION.SDK_INT>=33)requestPermissions(new String[]{android.Manifest.permission.POST_NOTIFICATIONS},701);AlertPush.initialize(this);AlertPush.schedule(this);});
 ...
 void render(JSONObject state)throws Exception{content.removeAllViews();status.setText(AlertPush.prefs(this).getString("pushStatus",""));JSONArray alerts=state.getJSONArray("alerts");if(alerts.length()==0)label("No alerts");
