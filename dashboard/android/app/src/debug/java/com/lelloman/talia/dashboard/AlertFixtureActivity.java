...
public final class AlertFixtureActivity extends Activity {
 ...
 void report(String status){try{JSONArray notifications=new JSONArray();for(android.service.notification.StatusBarNotification n:getSystemService(NotificationManager.class).getActiveNotifications())notifications.put(new JSONObject().put("key",n.getTag()).put("title",n.getNotification().extras.getString(Notification.EXTRA_TITLE)).put("actions",n.getNotification().actions==null?0:n.getNotification().actions.length));JSONObject r=new JSONObject().put("status",status).put("installation",AlertPush.installation(this)).put("deviceVersion",AlertPush.prefs(this).getLong("deviceVersion",0)).put("notifications",notifications);try(java.io.FileOutputStream out=openFileOutput("alert-fixture.json",0)){out.write(r.toString().getBytes(StandardCharsets.UTF_8));}}catch(Exception ignored){}}
}
